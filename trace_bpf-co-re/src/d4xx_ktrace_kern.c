...
/*
*/
SEC("kprobe/ds5_mux_s_stream")
int BPF_KPROBE(ds5_mux_s_stream_entry, struct v4l2_subdev___local *sd, int on)
{
	struct ds5___local *state = container_of(sd, struct ds5___local, mux.sd.subdev);

	struct ds5_sensor___local *sensor = (struct ds5_sensor___local *)BPF_CORE_READ(state,mux.last_set);
	...
	if (mux_pad != DS5_MUX_PAD_EXTERNAL &&
	    mux_pad < DS5_MUX_PAD_COUNT) {
	  // this is DS4XX
	  bpf_printk("graph_ent func=ds5_mux_s_stream args=name=%s;on=%d",
		      d4xx_mux_pad_types[mux_pad], on);
	} else {
	  bpf_printk("graph_ent func=ds5_mux_s_stream args=name=%s;on=%d",
		      d4xx_mux_pad_types[mux_pad], on);
	}
...
SEC("kretprobe/ds5_mux_s_stream")
int BPF_KRETPROBE(ds5_mux_s_stream_exit, int ret)
{
	bpf_printk("graph_ret func=ds5_mux_s_stream ret=%d", ret);
	return 0;
}
