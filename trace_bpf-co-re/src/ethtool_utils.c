...
int ethtool_get_drvinfo(const char *ifname, char *drvname, size_t num)
{
	struct ethtool_drvinfo d = { .cmd = ETHTOOL_GDRVINFO };
	struct ifreq ifr = {};
	int fd, err, ret = -1;
	...
	if (err) {
		/* If the device says it has no driver
		 */
		ret = 1;
		goto out;
	}

	strncpy(drvname, d.driver, num);
	ret = 1;
